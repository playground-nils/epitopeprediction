...
For **netMHCpan** predictions specifically, the pipeline uses **EL_Rank (Eluted Ligand Rank)** by default, which is the rank metric recommended by the netMHCpan developers. EL_Rank is computed against a reference set of eluted ligands rather than binding affinity measurements. This means that the rank values are not directly derived from the BA (Binding Affinity) scores shown in the output. If you prefer to use BA_Rank (Binding Affinity Rank) instead, which is computed against binding affinity measurements and correlates more directly with the BA column, you can enable the `--use_ba_rank` parameter. Note that EL_Rank and BA_Rank can differ significantly, and EL_Rank is generally recommended for identifying immunogenic peptides.
