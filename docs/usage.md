...
The pipeline supports NetMHCpan 4.2bstatic and NetMHCIIpan 4.3e. If one of the external tools is specified, the path to the corresponding tarball has to be specified. See the download sections for [NetMHCpan-4.2](https://services.healthtech.dtu.dk/services/NetMHCpan-4.2/) and [NetMHCIIpan-4.3](https://services.healthtech.dtu.dk/services/NetMHCIIpan-4.3/). When using `conda`, the parameter `--netmhc_system` (if the default value `linux` is not applicable) must also be specified.
