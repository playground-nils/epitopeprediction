...
class PredictionResult:
    ...
    def _format_netmhcpan_prediction(self) -> pd.DataFrame:
        ...
        # Extract Peptide, percentile rank (EL_Rank), binding affinity
    ...
    def _format_netmhciipan_prediction(self) -> pd.DataFrame:
        ...
        `Peptide,Rank_EL,Score_BA` for multiple alleles.
        """
        ...
        df = pd.read_csv(self.file_path, sep='\t', skiprows=1)
        # Extract Peptide, percentile rank (EL_Rank), binding affinity
        df = df[df.columns[df.columns.str.contains('Peptide|Rank_EL|Score_BA')]]

        df = df.rename(columns={'Peptide': self.peptide_col_name, 'Rank_EL': 'Rank_EL.0', 'Score_BA': 'Score_BA.0'})
        # to longformat based on .0|1|2..
        df_long = pd.melt(
            df,
            id_vars=[self.peptide_col_name],
        ...
        # Extract the allele information (e.g., .0, .1, etc.)
        df_long['allele'] = df_long['metric'].str.split('.').str[1]
        df_long['metric'] = df_long['metric'].apply(lambda x: x.split('.')[0].replace('Rank_EL', 'rank').replace('Score_BA', 'BA'))
