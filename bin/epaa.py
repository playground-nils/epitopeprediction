...
def generate_fasta_output(output_filename: str, mutated_proteins: list, mutated_peptides_df: pd.DataFrame, flanking_region_size: int, martsadapter: MartsAdapter = None):
    """
    ...
    Args:
        ...
        martsadapter (MartsAdapter, optional): BioMart adapter for fetching missing wildtype sequences.
    """
    global vcfConsequences
    ...
    # Fetch missing wildtype sequences from BioMart / Ensembl REST API
    # Wildtype proteins are normally generated by epytope when heterozygous variants create
    # a "no-variant" combination path. However, when a transcript has >10 variants, all variants
    # are forced to homozygous to prevent combinatorial explosion (2^N), which means no wildtype
    # protein is generated. We fix this by independently fetching the reference sequence.
    missing_wt_tids = [tid for tid, entry in fasta_dict.items() if entry["seq_wt"] is None]
    if missing_wt_tids:
        logger.info(f"{len(missing_wt_tids)} transcript(s) are missing wildtype sequences. Fetching from BioMart/Ensembl...")
        fetched_biomart = 0
        fetched_rest = 0
        failed = []
        for tid in missing_wt_tids:
            # Try 1: BioMart via MartsAdapter
            if martsadapter is not None:
                try:
                    cds = martsadapter.get_transcript_sequence(tid, type=ID_SYSTEM_USED)
                    if cds:
                        protein_seq = str(Seq(cds).translate(to_stop=True))
                        if protein_seq:
                            fasta_dict[tid]["seq_wt"] = protein_seq
                            fetched_biomart += 1
                            continue
                except Exception as e:
                    logger.debug(f"BioMart lookup failed for {tid}: {e}")
            # Try 2: Ensembl REST API (handles transcripts not found via BioMart XML queries)
            try:
                import urllib.request
                url = f"https://rest.ensembl.org/sequence/id/{tid}?type=cds"
                req = urllib.request.Request(url, headers={"Content-Type": "text/plain"})
                with urllib.request.urlopen(req, timeout=30) as response:
                    cds = response.read().decode("utf-8").strip()
                if cds:
                    protein_seq = str(Seq(cds).translate(to_stop=True))
                    if protein_seq:
                        fasta_dict[tid]["seq_wt"] = protein_seq
                ...
                logger.debug(f"Ensembl REST API lookup failed for {tid}: {e}")
            failed.append(tid)
        if fetched_biomart or fetched_rest:
            logger.info(f"Successfully fetched wildtype sequences: {fetched_biomart} from BioMart, {fetched_rest} from Ensembl REST API.")
        if failed:
            logger.warning(f"{len(failed)} transcript(s) still missing wildtype sequences: {', '.join(failed[:10])}{'...' if len(failed) > 10 else ''}")
...
def __main__():
    ...
    if args.fasta_output:
        generate_fasta_output(f"{args.prefix}.fasta", mutated_proteins, mutated_peptides_df, args.flanking_region_size, martsadapter)
