...
def generate_fasta_output(output_filename: str, mutated_proteins: list, mutated_peptides_df: pd.DataFrame, flanking_region_size: int, martsadapter: MartsAdapter = None):
    ...
    # Fetch missing wildtype sequences from BioMart
    ...
    if missing_wt_tids:
        logger.info(f"{len(missing_wt_tids)} transcript(s) are missing wildtype sequences. Fetching from BioMart...")
        fetched = 0
        failed = []
        for tid in missing_wt_tids:
            ...
            failed.append(tid)
        if fetched:
            logger.info(f"Successfully fetched {fetched} wildtype sequence(s) from BioMart.")
        if failed:
            logger.warning(f"{len(failed)} transcript(s) still missing wildtype sequences after BioMart lookup: {', '.join(failed[:10])}{'...' if len(failed) > 10 else ''}")
